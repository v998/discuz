...
function setEditorEvents() {
	var floatPic = function(e) {
		...
		if(!menu) {
			...
			menu.innerHTML = '<div class="popupmenu_option" unselectable="on"><ul unselectable="on"><li id="' + menuid + '_left" unselectable="on">圖片居左混排</li><li id="' + menuid + '_right" unselectable="on">圖片居右混排</li></ul></div>';
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'createlink':
				str = '請輸入鏈接的地址:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="txt" />'+
					(selection ? '' : '<br />請輸入鏈接的文字:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="txt" />');
				break;
			case 'forecolor':
			...
			case 'free':
				...
				var lang = {'quote' : '請輸入要插入的引用', 'code' : '請輸入要插入的代碼', 'hide' : '請輸入要插入的隱藏內容', 'free' : '請輸入要插入的免費信息'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="txt" checked="checked" />只有當瀏覽者回復本帖時才顯示<br /><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="txt" />只有當瀏覽者積分高於 <input type="text" size="3" id="' + ctrlid + '_param_2" class="txt" /> 時才顯示' : '');
				break;
			case 'table':
				str = '表格行數: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="txt" /> &nbsp; 表格列數: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="txt" /><br />表格寬度: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="txt" /> &nbsp; 背景顏色: <input type="text" id="' + ctrlid + '_param_4" size="2" class="txt" onclick="showColorBox(this.id, 2)" />';
				break;
			case 'audio':
				str = '請輸入音樂文件地址:<br /><input type="text" id="' + ctrlid + '_param_1" class="txt" value="" style="width: 245px;" />';
				break;
			case 'video':
				str = '請輸入視頻地址:<br /><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 245px;" class="txt" /><br />寬: <input id="' + ctrlid + '_param_2" size="5" value="400" class="txt" /> &nbsp; 高: <input id="' + ctrlid + '_param_3" size="5" value="300" class="txt" />';
				break;
			case 'flash':
				str = '請輸入 Flash 文件地址:<br /><input type="text" id="' + ctrlid + '_param_1" class="txt" value="" style="width: 245px;" />';
				break;
			default:
				...
				for(var i = 1; i <= params; i++) {
					if(i != params || !haveSel) {
						str += (promptlang[i - 1] ? promptlang[i - 1] : '請輸入第 ' + i + ' 個參數:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="txt" />' + (i < params ? '<br />' : '');
					}
				}
				break;
		}
...
function insertAttach(prefix, id) {
	...
	$(prefix + 'cpdel_' + id).innerHTML = '<a href="###" class="deloption" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">刪除</a>';
...
function updateattachnum(type) {
	...
	if(num) {
		$(editorid + '_cmd_' + type).title = '包含 ' + num + (type == 'image' ? ' 個圖片附件' : ' 個附件');
		$(editorid + '_cmd_' + type + '_notice').style.display = '';
	} else {
		$(editorid + '_cmd_' + type).title = type == 'image' ? '圖片' : '附件';
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		showDialog('上傳失敗:' + STATUSMSG[arr[2]]);
	}
}
